...
class Application @Inject() extends Controller {
 ...
 val driver = new MongoDriver
  ...
  val scRr = new SparkContext("local[*]", "RecommenderApp","8g")
  scRr.addJar("target/scala-2.11/blog-spark-recommendation_2.11-2.0-SNAPSHOT.jar")
  println(scRr)
  
  val recommender = new Recommender(scRr, RatingFile)
 
 /* def index = Action {
    Ok("Welcome in  world of SPARK")
  }*/
...
//inserting entry of ratings in mongoDB database: Data comming from /rating?item=@item.itemId&rating=1(rating value change as per click)
